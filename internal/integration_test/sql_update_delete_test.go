...
func TestSQLUpdateStatement(t *testing.T) {
	...
	t.Run("UPDATE single column with WHERE", func(t *testing.T) {
		...
		initialEmail := getRowMap(result.Rows[0], result.Columns)["email"]
		...
		newEmail := getRowMap(result.Rows[0], result.Columns)["email"]
	...
	t.Run("UPDATE multiple columns", func(t *testing.T) {
		...
		if len(result.Rows) > 0 {
			rowMap := getRowMap(result.Rows[0], result.Columns)
			if rowMap["username"] != "multiuser" {
				t.Errorf("Expected username 'multiuser', got '%v'", rowMap["username"])
			}
			if rowMap["email"] != "multi@test.com" {
				t.Errorf("Expected email 'multi@test.com', got '%v'", rowMap["email"])
			}
		}
	})
...
func TestSQLCombinedOperations(t *testing.T) {
	...
	t.Run("INSERT then UPDATE then DELETE", func(t *testing.T) {
		...
		result, _ = eng.Execute(selectSQL)
		if len(result.Rows) > 0 && getRowMap(result.Rows[0], result.Columns)["email"] != "updated@example.com" {
