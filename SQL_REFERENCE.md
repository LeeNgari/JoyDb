...
9. **DDL fully WAL-logged**: CREATE TABLE, DROP TABLE, and ALTER TABLE (ADD_COLUMN/DROP_COLUMN) DDL statements are fully logged to the Write-Ahead Log (WAL v2). Upon recovery, DDL schemas are replayed before DML queries, guaranteeing full schema durability and structural consistency on database startup.
